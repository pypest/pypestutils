! -- Notes
!    (a) Are there places where I should be using intent(inout) instead of undecared intent?
...
250          format(t2,i8,t12,i8,t22,a16,t42,i8,t52,i8,t62,i8)
           ...
           if(lout)then
             if(imeth.eq.6)then    ! mf6 and therefore double precision
               ...
               adjustr(txt1id1),adjustr(txt2id1),adjustr(txt2id1),adjustr(txt2id2)
...
40       format('A maximum of ',a,' structured model grids can be installed. ',   &
         'You must uninstall at least one structured model grid before you ',     &
...
135      format('At least one element of the ',a,' array is non-positive.')
       ...
       if((rotation.gt.180.0d0).or.(rotation.lt.-180.0d0))then
         write(amessage,140)
140      format('Input argument ROTATION must not be less than -180 or ',  &
         'greater than 180.')
         go to 9890
       end if
...
integer function interp_from_structured_grid(                                         &
...
! -- This function performs spatial interpolation from a structured grid to a set of points.
       ...
       integer                        :: iarray,itime,mcol,mrow,ilay,ibig
...
40       format('A maximum of ',a,' MODFLOW 6 model grid specifications ',  &
         ...
         'grid specifications before you can install a new one.')
         go to 9890
       end if
...
! -- We read the 4 header lines.
        ...
        atype='header'
        do i=1,4
          read(gridunit,err=9000,end=9050) header(i)
        end do

! -- We obtain data from these header lines.

        header(1)=adjustl(header(1))
        if(header(1)(1:9).eq.'GRID DIS ')then
          mf6modgrid(igrid)%distype=1
        else if(header(1)(1:9).eq.'GRID DISV')then
          mf6modgrid(igrid)%distype=2
        else if(header(1)(1:9).eq.'GRID DISU')then
          mf6modgrid(igrid)%distype=3
        else
          write(amessage,120) trim(header(1)(1:49)),trim(afile1)
...
! -- Find cell centre coords in local coordinate system.
        ...
        else    ! The following assumes that there are no "stray vertices".
          xxmin=1.0d300
          xxmax=-1.0d300
          yymin=1.0d300
          yymax=-1.0d300
...
120      format('The ',a,' argument of function ',a,' must not be blank.')
       ...
       if((factorfiletype.ne.0).and.(factorfiletype.ne.1))then
         write(amessage, 130) trim(function_name)
130      format('The FACFILETYPE argument of function ',a,' must be supplied as 0 or 1.')
         go to 9890
       end if
       if(interpthresh.le.0.0d0)then
