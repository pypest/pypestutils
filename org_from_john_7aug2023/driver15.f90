...
! -- DRIVER15 tests 3-D random field generation.
...
! -- Variables used in function calls.
       ...
       real(kind=c_double), allocatable   :: xx(:),yy(:),zz(:)
       real(kind=c_double), allocatable   :: mean(:),area(:),height(:),var(:)
...
! -- Read specifications of general grid.

100     write(6,110,advance='no')
110     format(' Enter name of 3D node specifications file: ')
        read(5,'(a)') afile
...
! -- See if the column headers are ok.
        ...
        call linesplit(jfail,8,lw,rw,cline)
        ...
        if(atemp(1:1).ne.'z') go to 9050
        atemp=cline(lw(5):rw(5))
        if(atemp(1:3).ne.'lay') go to 9050
        atemp=cline(lw(6):rw(6))
        if(atemp(1:4).ne.'area') go to 9050
        atemp=cline(lw(7):rw(7))
        if(atemp.ne.'height') go to 9050
        atemp=cline(lw(8):rw(8))
...
! -- Allocate memory.
        ...
        allocate(xx(nnode),yy(nnode),zz(nnode),layer(nnode),area(nnode),    &
        height(nnode),iarray(nnode),stat=ierr)
...
210         format('Node numbers should start at 1 and be sequential. This is ',  &
          ...
          if(jfail.ne.0) go to 9000
          call drealread(jfail,cline(lw(4):rw(4)),zz(jnode))
          if(jfail.ne.0) go to 9000
          call intread(jfail,cline(lw(5):rw(5)),layer(jnode))
          if(jfail.ne.0) go to 9000
          call drealread(jfail,cline(lw(6):rw(6)),area(jnode))
          if(jfail.ne.0) go to 9000
          call drealread(jfail,cline(lw(7):rw(7)),height(jnode))
          if(jfail.ne.0) go to 9000
          call intread(jfail,cline(lw(8):rw(8)),iarray(jnode))
          if(jfail.ne.0) go to 9000
...
! -- Allocate more memory.
        ...
        allocate(mean(nnode),var(nnode),ahmax(nnode),ahmin(nnode),    &
        avert(nnode),bearing(nnode),dip(nnode),rake(nnode),stat=ierr)
        if(ierr.ne.0) go to 9200
...
260     format(' Enter name of 3D averaging function specification file: ')
        read(5,'(a)') afile
...
! -- See if the column headers are ok.
        ...
        call linesplit(jfail,9,lw,rw,cline)
        ...
        atemp=cline(lw(4):rw(4))
        if(atemp.ne.'a_hmax') go to 9070
        atemp=cline(lw(5):rw(5))
        if(atemp.ne.'a_hmin') go to 9070
        atemp=cline(lw(6):rw(6))
        if(atemp.ne.'a_vert') go to 9070
        atemp=cline(lw(7):rw(7))
        if(atemp.ne.'bearing') go to 9070
        atemp=cline(lw(8):rw(8))
        if(atemp.ne.'dip') go to 9070
        atemp=cline(lw(9):rw(9))
        if(atemp.ne.'rake') go to 9070
...
! -- Now read the file.
        ...
        do
          ...
          if(jfail.ne.0) go to 9000
          call drealread(jfail,cline(lw(4):rw(4)),ahmax(jnode))
          if(jfail.ne.0) go to 9000
          call drealread(jfail,cline(lw(5):rw(5)),ahmin(jnode))
          if(jfail.ne.0) go to 9000
          call drealread(jfail,cline(lw(6):rw(6)),avert(jnode))
          if(jfail.ne.0) go to 9000
          call drealread(jfail,cline(lw(7):rw(7)),bearing(jnode))
          if(jfail.ne.0) go to 9000
          call drealread(jfail,cline(lw(8):rw(8)),dip(jnode))
          if(jfail.ne.0) go to 9000
          call drealread(jfail,cline(lw(9):rw(9)),rake(jnode))
          if(jfail.ne.0) go to 9000
          if(jnode.eq.nnode) exit
        end do
        close(unit=10)
...
! -- Acquire number of realisations.
...
! -- Call the function to fill the array.
        ...
        write(6,*)
        write(6,421)
421     format(' Calling fieldgen3d_sva()....')
        ifail=fieldgen3d_sva(            &
              nnode,                     &
              xx,yy,zz,                  &
              area,height,iarray,        &
              mean,var,                  &
...
9080    format('Header on first line of file ',a,' expected to be ',   &
        '"node, mean, var, a_hmax, a_hmin, a_vert, bearing, dip, rake".')
        go to 9890
...
9250    write(amessage,9260)
9260    format('Memory deallocation error. Cannot continue execution.')
        go to 9890
...
! -- Free local memory
       ...
       deallocate(xx,yy,zz,stat=ierr)
       deallocate(mean,area,height,var,stat=ierr)
