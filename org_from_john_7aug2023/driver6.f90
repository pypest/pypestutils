...
! -- DRIVER6 tests 2-D kriging interpolation
...
! -- Allocatable variables; first the ones that are used in function calls.
       ...
       real(kind=c_double), allocatable   :: aa(:),mean(:),anis(:),bearing(:)
...
! -- Allocate target arrays.
       ...
       allocate(aa(mpts),mean(mpts),anis(mpts),bearing(mpts),znt(mpts),stat=ierr)
       if(ierr.ne.0) go to 9200
...
150      format(' Enter name of ',a,' array file: ')
         ...
         if(ifile.eq.1)then
           read(10,*,err=9000,end=9050) (aa(ipt),ipt=1,mpts)
         else if(ifile.eq.2)then
...
206    format(' Enter variogram type (1=spher;2=exp;3=gaus;4=pow): ')
       read(5,*,err=205) vartype
210    write(6,220,advance='no')
220    format(' Enter search radius: ')
       read(5,*,err=210) searchrad
222    write(6,223,advance='no')
223    format(' Enter minimum number of points to use in kriging: ')
       read(5,*,err=222) minpts
224    write(6,225,advance='no')
225    format(' Enter maximum number of points to use in kriging: ')
       read(5,*,err=224) maxpts
...
320    format(' Calling calc_kriging_factors_2d()....')
       ifail=calc_kriging_factors_2d(npts,ecs,ncs,zns,                      &
                                   mpts,ect,nct,znt,                        &
                                   vartype,krigtype,                        &
                                   aa,anis,bearing,                         &
...
! -- Free local memory
       ...
       deallocate(aa,mean,anis,bearing,stat=ierr)
