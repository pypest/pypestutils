...
! -- The main purpose of this driver program is to test the reading of a cell-by-cell
!    flow term file.

       implicit none

       integer           :: inquire_modflow_binary_file_specs, &
                            extract_flows_from_cbc_file,       &
                            interp_to_obstime,                 &
                            retrieve_error_message,            &
                            free_all_memory

       integer           :: ierr,ifail,itemp
       integer           :: ncell,icell,nzone
       integer           :: isim
       integer           :: itype,iprec,narray,ntime
       integer           :: numzone,nproctime
       integer           :: iz,itime
       integer           :: numobsdat,iobs

       double precision  :: time_extrap,nointerpval,interpthresh

       character (len=1)    :: how_extrap,aa
       character (len=1)    :: flowtype(17)
       character (len=1)    :: cbcfile(256),recfile(256)
       ...
       integer, allocatable :: zonenumber(:)
       integer, allocatable :: timestep(:),stressperiod(:)
       integer, allocatable :: obszonenum(:)
       double precision, allocatable     :: simtime(:),simflow(:,:)
...
290      format(' - function call unsuccessful; error message follows.')
       ...
       if(itype.eq.1)then
         write(amessage,300) trim(afile1)
300      format('File ',a,' is not a cbc flow term file.')
         go to 9890
       end if
       write(6,*)
           ...
           if(ifail.eq.0)then
             do iz=1,numzone
               if(itemp.eq.zonenumber(iz))then
                 obszonenum(iobs)=iz-1             ! Don't forget requires indexing beging at 0
                 go to 510
               end if
             end do
           end if
